...
// Função para calcular a semana do ano corretamente
...
// Função para converter data string no formato 'DD/MM/YYYY' para objeto Date
  ...
  useEffect(() => {
    if (data) {
      const groupedByWeek = data.reduce((acc, item) => {
        const date = parseDate(item.label);
